...
class AccessService {
  ...
  async findPublicPortalPersonRecovery(session, searchName, t) {
    const tx = resolveTranslate(t);
    const profile = await this.getPortalProfileOrFail(session, tx);
    const compactName = String(searchName || '')
      .trim()
      .toLowerCase()
      .replace(/\s+/g, '');

    if (compactName.length < 4) {
      throw new AppError(tx('validation.portal.fullName', { min: 2, max: 160 }), 422);
    }

    const [passRows, wristbandRows, auditRows] = await Promise.all([
      this.pool.execute(
        `
          SELECT
            'pass' AS request_type,
            pr.id,
            pr.request_profile_id,
            rp.access_code AS profile_access_code,
            rp.name AS profile_name,
            pr.pass_category_id AS category_id,
          ...
          LEFT JOIN pass_categories pc ON pc.id = pr.pass_category_id
          WHERE pr.event_id = ?
            AND LOWER(REPLACE(TRIM(pr.full_name), ' ', '')) LIKE ?
          ORDER BY pr.deleted_at IS NOT NULL, pr.created_at DESC, pr.id DESC
          LIMIT 50
        `,
        [profile.event_id, `%${compactName}%`],
      ).then(([rows]) => rows),
      this.pool.execute(
        `
          SELECT
            'wristband' AS request_type,
            wr.id,
            wr.request_profile_id,
            rp.access_code AS profile_access_code,
            rp.name AS profile_name,
            wr.wristband_category_id AS category_id,
          ...
          LEFT JOIN wristband_categories wc ON wc.id = wr.wristband_category_id
          WHERE wr.event_id = ?
            AND LOWER(REPLACE(TRIM(wr.full_name), ' ', '')) LIKE ?
          ORDER BY wr.deleted_at IS NOT NULL, wr.created_at DESC, wr.id DESC
          LIMIT 50
        `,
        [profile.event_id, `%${compactName}%`],
      ).then(([rows]) => rows),
      this.pool.execute(
        `
          ...
          WHERE event_id = ?
            AND (
              LOWER(REPLACE(COALESCE(message, ''), ' ', '')) LIKE ?
              OR LOWER(REPLACE(COALESCE(before_state, ''), ' ', '')) LIKE ?
              OR LOWER(REPLACE(COALESCE(after_state, ''), ' ', '')) LIKE ?
            )
          ORDER BY created_at DESC
          LIMIT 50
        `,
        [profile.event_id, `%${compactName}%`, `%${compactName}%`, `%${compactName}%`],
      ).then(([rows]) => rows),
    ]);
